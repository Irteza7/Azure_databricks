...
race_results_df = results_df.join(drivers_df, results_df.driver_id == drivers_df.driver_id, "inner")\
    ...
    .select(results_df.race_id, races_circuits_df.race_year, races_circuits_df.race_name, races_circuits_df.race_date, races_circuits_df.circuit_location, drivers_df.name.alias("driver_name"), drivers_df.number.alias("driver_number"), drivers_df.nationality.alias("driver_nationality"), constructors_df.name.alias("team"), results_df.grid, results_df.fastest_lap, results_df.time.alias("race_time"), results_df.points, results_df.position, results_df.file_date).withColumn("created_date", current_timestamp())
...
df_inc_write_to_table("f1_presentation","race_results",race_results_df,"race_id")
